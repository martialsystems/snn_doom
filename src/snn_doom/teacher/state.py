...
def pack_input(turn_left: int, turn_right: int, fwd: int, back: int) -> int:
    for v in (turn_left, turn_right, fwd, back):
        if v not in (0, 1):
            raise ValueError("input bits must be 0 or 1")
    return turn_left | (turn_right << 1) | (fwd << 2) | (back << 3)


def unpack_input(bits: int) -> tuple[int, int, int, int]:
