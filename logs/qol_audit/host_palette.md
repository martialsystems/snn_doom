...
- cluster HOSTPAL: Python blit a warmer wall palette and a bigger HUD font
- teacher_delta does not name a teacher field or apply_* function
- zeroing HOSTPAL must kill only the new job; CLOCK/LATCH/REG/ALU/RAM/RAY/READOUT/DOOR rows in logs/ablation.json must keep their isolation shape
...
- leave PALETTE in demo/view.py (0 neurons) (0 neurons). not cheaper than the proposal

Ablation contract: zeroing HOSTPAL must kill only the new job; CLOCK/LATCH/REG/ALU/RAM/RAY/READOUT/DOOR rows in logs/ablation.json must keep their isolation shape
