...
This repo does the other, worse thing.

It takes a pile of leaky integrate-and-fire neurons, strips the fly out of them, and makes the pile be the engine. Not a joystick. Not a policy. Not "each frame stimulates ommatidia." The net holds the map, the pose, the enemy, the march, and the framebuffer. Python is allowed to poke four keys, step the circuit, and draw whatever the color lines say. If Python raycasts, we failed.

It is a 7,217-neuron insult to both neuroscience and id Software. It runs at about 1.6 ticks per second. The walls show up. The distances are still a little drunk.

That is the product.

## What this is not

Not ViZDoom with extra steps.
Not a fly. Courtship song does not fire the shotgun. There is no shotgun.
Not commercial DOOM.WAD. The world is an 8×8 postage stamp with one guy who wants to stand near you.
Not 166,700 neurons. That number is a threat we have not earned. RAY is not bit-exact yet. Until it is, we stay small and mean.
...
Ablations: zero CLOCK, LATCH, REG, ALU, or SEQUENCER and motion dies. Zero RAY or READOUT and pixels die. Zero RAM and the frame changes. LATCH still looks too quiet if no key is held. That is a test gap, not a feature.
...
`--fwd` sets the forward bit in the recorded path. The interactive GUI is for humans. CI does not owe you a window.

## Methodology

The rest of this file is the machine, not the joke.
...
World: 8×8 cells, 16 subcells/cell (4.4 fixed point). Angle: 64 ticks = 360°. Map bit 1 is wall. Frame: 16×16, 2-bit color {sky, floor, wall, enemy}. Rays: 16 columns, angles ang-8 … ang+7, march 8 units/step, max 15. Height: 16 - dist.
...
| Module | Job | Notes |
|--------|-----|-------|
| CLOCK | SETTLE-period ring | kick packet is 6 steps; that was a bug once |
...
Those four are process laws. Pytest is evidence. Do not report "done" on a scale-up that skips RAY lock.
...
Until then this is a small cursed computer that almost draws corridors.
