...
Input bits (host injects every LIF step of a game tick): turn_left, turn_right, fwd, back.
...
Host: inject 4 key bits, step `STEPS_PER_TICK` LIF updates, argmax 4 color lines per pixel, draw, log.
